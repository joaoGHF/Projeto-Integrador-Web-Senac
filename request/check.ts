...
export function isCPF(_text:any): boolean {

    if ( !isString(_text) ) {
        return false;
    }

    const check = /^\d{3}\.\d{3}\.\d{3}\-\d{2}$/;
...
export function isNumber(_value:any): boolean {

    if ( _value instanceof Number || typeof _value === 'number' ) {
        return true;
    }
    else {
        return false;
